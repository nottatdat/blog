...
* **Trang 24 - 26**:
    ...
    Thay vì thêm *"nhiễu"* một cách "thô thiển" với biên độ bằng nhau ở các lớp, *[OpenAI](https://openai.com/)* đề xuất một phương pháp thêm *"nhiễu"* khá thú vị, mà cũng rất tự nhiên: Ở các lớp đầu tiên thì biên độ của *"nhiễu"* được thêm vào mạnh hơn, và biên độ *"nhiễu"* sẽ giảm dần theo độ sâu của mạng. Sự thay đổi về biên độ này xuất phát từ việc: Ở một mạng neuron đủ *"sâu"*, các lớp khác nhau sẽ có sự *"nhạy cảm"* khác nhau với cùng một biên độ *"nhiễu"*. Không chỉ thế, *[OpenAI](https://openai.com/)* cũng đề xuất một phương pháp để từ sai số của mô hình mà có thể tinh chỉnh nhiễu ở từng lớp cho phù hợp.
